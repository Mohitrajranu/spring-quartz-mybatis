					...
					<tr>
						...
						<td>${job.jobStatus }<c:choose>
								<c:when test="${job.jobStatus=='1' }">
									<a href="javascript:;"
										onclick="changeJobStatus('${job.jobId}','stop')">stop</a>&nbsp;
								</c:when>
								<c:otherwise>
									<a href="javascript:;"
										onclick="changeJobStatus('${job.jobId}','start')">start</a>&nbsp;
								</c:otherwise>
							</c:choose>
						</td>
						...
						<td><a href="javascript:;" onclick="updateCron('${job.jobId}')">Updatecron</a></td>
					</tr>
				</c:forEach>
				<tr>
					<td>n</td>
	...
	<script>
		function validateAdd() {
			...
			if ($.trim($('#beanClass').val()) == '' && $.trim($('#springId').val()) == '') {
				$('#beanClass').focus();
				alert('At least one of the classpath and spring id ');
				return false;
			}
		...
		function updateCron(jobId) {
			var cron = prompt("Enter cron expression! ", "")
			if (cron) {
				showWaitMsg();
